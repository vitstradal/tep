class Act::EventParticipantsController < ActController
  ...
  def delete
    ...
    if (! can? :delete_other, Act::EventParticipant) && @event_participant.participant.user.id != current_user.id
      render :not_allowed, locals: { desired: "upravovati " }
      return
    end
  ...
  def choose
    @event_participant = Act::EventParticipant.find_by(event_id: params[:event_id], participant_id: params[:participant_id])
    if (! can? :update_other, Act::EventParticipant) && @event_participant.participant.user.id != current_user.id
      render :not_allowed, locals: { desired: "upravovati " }
      return
    end
