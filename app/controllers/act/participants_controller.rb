...
class Act::ParticipantsController < ActController  
  ...
  def index
    ...
    when "young"
      @participants = Act::Participant.find_by_sql(["SELECT * FROM act_participants WHERE grade < ?", Act::Participant::YOUNGEST_TO_FILTER])
    when "old"
      @participants = Act::Participant.find_by_sql(["SELECT * FROM act_participants WHERE grade > ?", Act::Participant::OLDEST_TO_FILTER])
    else
  ...
  def show
    ...
    if !(can? :show_other, Act::Participant) && !(Act::Participant::participant_id(current_user) == params[:participant_id].to_i)
  ...
  def new
    if current_user.nil?
      ...
      return
    elsif Act::Participant::has_participant?(current_user) 
      redirect_to act_participant_path(current_user.participant)
      return
    end
  ...
  def create_other
    if ! can? :create_other, Act::Participant
