class EventParticipantsController < ApplicationController
  def enroll_other
    @participant = EventParticipant.find_by(event_id: params[:event_id], scout_id: params[:scout_id])

    if @participant.nil? && (! can? :create_other, EventParticipant)
      render :not_allowed, locals: { desired: "vytvářeti " }
      return
    end

    if current_user.nil? || (! can? :update_other, EventParticipant) && @participant.scout.user.id != current_user.id
      render :not_allowed, locals: { desired: "upravovati " }
      return
    end

    if @participant.nil?
      @participant = EventParticipant.new(event_id: params[:event_id], scout_id: params[:scout_id], status: "yes", chosen: params[:chosen])
  ...
  def delete
    ...
    if (! can? :delete_other, EventParticipant) && @participant.scout.user.id != current_user.id
      render :not_allowed, locals: { desired: "upravovati " }
      return
    end
  ...
  def choose
    @participant = EventParticipant.find_by([params[:event_id], params[:scout_id]])
    if (! can? :update_other, EventParticipant) && @participant.scout.user.id != current_user.id
      render :not_allowed, locals: { desired: "upravovati " }
      return
    end
